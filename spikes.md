...
**Decision log:**
- Decision: implemented `osReducer` as a discriminated union of 20 actions (16 from §3 plus 4 new — see below), plus `OSProvider`/`useOS` in `osContext.tsx`. **40 reducer unit tests pass**, and the provider was additionally driven in a real browser (11/11 interaction checks) rather than assumed to work.
- Notes:
  - **The most valuable finding of this spike is a layering bug, caught by a test I nearly wrote as a formality.** SPIKE-08 step 3 asks whether `nextZIndex` "overflows or causes visible glitches after many focus changes", so I asserted it stayed below the scanline layer. It failed: after 500 focus changes the top window's `zIndex` was **521**, versus `--z-scanlines: 50`, `--z-taskbar: 60`, `--z-startmenu: 70`. Left alone, windows would eventually render *over* the scanlines, the music widget, the taskbar, and the Start Menu — a bug that only shows up after minutes of ordinary clicking, i.e. exactly the kind that ships.
    - **Fix is CSS, not the counter:** `WindowManager` must establish a stacking context (`position: relative; z-index: var(--z-windows)`), which scopes per-window z-indexes inside the windows layer so unbounded growth is harmless. Renumbering all windows on every focus change would also work, but it's more reducer code and more re-renders for the same outcome. **This is now a hard requirement on Phase 2's `WindowManager`** and is written into `architecture.md` §5/§6 — if Phase 2 renders windows without that wrapper, the bug is back.
    - The test was rewritten to assert the invariant that's actually true (relative order matches focus order, counter stays a safe integer) with a comment explaining why asserting a ceiling would be asserting a bug.
  - **Four actions added beyond §3's list:** `RESTORE_WINDOW` (minimize is one-way — a minimized window has no title bar on screen, so only the taskbar tab can restore it, and it must un-minimize *and* focus), `CLOSE_START_MENU` (with only a toggle, the document-level outside-click handler would flip the menu back open), `SET_SHUTDOWN` (FR7's overlay, found in SPIKE-03), and `MUSIC_ERROR` (`lastError` had no way to be set). Each is justified in `architecture.md` §3 rather than just listed.
  - **`TOGGLE_MAXIMIZE` takes a `viewport` payload.** Maximized size depends on window dimensions, and I wanted the reducer to stay pure — no `window.innerWidth` reads — because that's what makes all 40 tests possible without a DOM. Same reason `MOVE_WINDOW` does no viewport clamping: only the drag hook knows the pointer offset, so clamping is SPIKE-06's job.
  - **Second bug caught by a test:** `RESTORE_WINDOW` initially delegated straight to the internal `focus()` helper, which short-circuits when the target is already `focusedWindowId`. With a single window open, minimizing then clicking its taskbar tab left it **stuck minimized forever**, because the id was still the focused one. Fixed by clearing `minimized` before focusing; there's a named regression test for it ("restores the last minimized window even when it was the focused one").
  - **Design calls made while implementing:**
    - **Focus is never dropped to `null` while windows remain.** Closing or minimizing the focused window hands focus to the topmost remaining visible window. The mockup doesn't do this (it only has one window), but losing focus to nowhere is exactly what SPIKE-28 has to clean up later, so it's cheaper to get right here.
    - **New windows cascade** 26px down-right, wrapping every 5. The mockup has one window at a fixed spot, so opening two apps would have stacked them pixel-identically and looked broken.
    - **Raising the volume from zero implicitly unmutes.** A slider that visibly moves while audio stays silent reads as a broken control. Sliding *to* zero deliberately does not touch the mute flag.
    - **Unknown-id actions return the identical state object** rather than throwing or creating a window. A `pointermove` can land after a window closes, and that shouldn't crash the desktop.
  - Verified the provider **in a real browser**, not only via unit tests: launched headless Chrome with `--remote-debugging-port` and drove it over the DevTools Protocol from a Node script (Node 22 has a global `WebSocket`, so this needed no new dependency and no Playwright/jsdom). It clicked through open → re-open → minimize → restore → close → close-last and read the rendered state dump each time: 11/11 passed, including live confirmation that taskbar tab order stays insertion-ordered while focus changes.
  - `App.tsx` is now a temporary state-inspector harness wrapped in `OSProvider` (the SPIKE-02 token-proof page it replaced had done its job). Both are throwaway — the real `Desktop` lands in Phase 2/3.
  - `useMemo` on the context value keeps `{ state, dispatch }` referentially stable between state changes; `useOS()` throws outside a provider rather than returning a silently-dead dispatch.
- Architecture.md impact: §3 — recorded the four new actions with reasons, the `viewport` payload on `TOGGLE_MAXIMIZE`, and the `OSProvider`/`useOS` contract. §5 — added the `WindowManager` stacking-context requirement, the cascade, one-window-per-app, focus-handoff behavior, and where the sizing constants live. §6 — corrected "windows (20+, dynamic)" to a single layer at 20, since the old phrasing is what invited the z-index bug.
...
**Decision log:**
- Decision (ship it / defer it): **split — ship music preference persistence, defer window layout persistence.** The spike framed this as one yes/no, but the two halves have very different cost/benefit and deserved different answers. Implemented in `src/state/persistence.ts` (28 unit tests) and wired into `OSProvider`; window layout is not written at all.
- Notes:
  - **Why window layout was cut, despite it being the headline feature of the original plan:**
    1. **It fights the design.** The site's entire premise is the desktop — cityscape, pixel icons, taskbar. A returning visitor restored into three open windows never sees it. The persistence "feature" would actively hide the thing being shown off.
    2. **Saved coordinates are viewport-relative and go stale.** A layout saved on a 2560px monitor reopens partly off-screen on a laptop, or after a window resize. Shipping layout persistence honestly means also shipping re-clamping-on-load logic — real complexity on the most intricate part of the state.
    3. **The audience visits once.** Recruiters and hiring managers are not returning power users; the "returning visitor" this optimises for is mostly me testing it.
    So it's the most complex thing to persist and the least valuable. Recorded in `architecture.md` §14 as an optional extension with the re-clamping caveat attached, so if it's ever revisited the cost is already written down.
  - **Why music preferences were kept:** four viewport-independent scalars (`videoId`, `isDefaultTrack`, `volume`, `isMuted`), no coordinate staleness, and a genuinely user-hostile failure mode if omitted — a visitor who turns the volume down or picks their own track, then refreshes, gets the default track back at 70%. SPIKE-22 step 3 already assumed this would exist.
  - **Explicitly not persisted, and this matters:** `isPlaying` and `awaitingUserGesture` are forced back to `false`/`true` on every load. A returning visitor gets no autoplay exemption — browser policy applies per page load regardless of stored preference (§8.3), so restoring "was playing" would produce a player whose UI claims it's playing while the browser has silently blocked it. `lastError` is dropped too. There's a test named for this so it isn't "tidied up" later by someone spreading the whole object.
  - **Treated stored data as untrusted input**, because that's what it is — anyone can edit it in devtools, and it can be from an older schema. Validation is **field-by-field, not all-or-nothing**: one corrupt value doesn't cost the visitor their other preferences. `videoId` is checked against `/^[A-Za-z0-9_-]{11}$/` before it can reach `loadVideoById`. Covered: missing key, malformed JSON, JSON that's a string/number/null/array, wrong types per field, non-finite and out-of-range volumes, and IDs of the wrong shape.
  - **Two failure modes that only exist in real browsers, both handled:** accessing `localStorage` *throws* in Safari private mode and when cookies are blocked (so even the property lookup is inside `try`/`catch`, not just the `getItem` call), and `setItem` throws on quota exhaustion. Both degrade to a no-op — losing a volume preference must never break the desktop. Tested with a throwing Storage stub and with no `localStorage` at all.
  - Schema version is **in the key** (`portfolio-os:music:v1`) rather than a `version` field inside the payload, so a future shape change just moves to `:v2` and stale data is ignored — no migration code to write or test. Loads also *merge* over defaults rather than replacing, so a field added to `MusicState` later still gets its default for a visitor with an older blob.
  - Hydration runs through `useReducer`'s **third (lazy initializer) argument**, so the `localStorage` read happens once rather than on every render of `OSProvider`. Passing `initial` explicitly bypasses hydration entirely, which keeps tests deterministic.
  - Saves are debounced 300ms — dragging a volume slider fires a change per pixel, and each one would otherwise be a synchronous `localStorage` write.
  - **Verified in a real browser, not just against the stub.** The unit tests use a `Map`-backed `Storage`, which by construction can't prove anything about actual navigation. Drove Chrome over CDP through: fresh load → change track/volume/mute → open a window → wait out the debounce → inspect the raw stored blob → **real `location.reload()`**. 11/11 passed, confirming preferences come back, that `isPlaying`/`awaitingUserGesture` do *not*, that the desktop is clean with no restored windows, that only the four intended fields are written, and that a hand-corrupted blob falls back to defaults instead of breaking the page.
  - `clearPersistedState()` is exported but intentionally not wired to any UI — it's an escape hatch for a visitor with a wedged stored value and a reset for tests. Flagged in case a "reset preferences" control is ever wanted.
- Architecture.md impact: §3 — replaced the "Persistence (optional)" paragraph with the actual split decision, the exact fields, the never-restore rule, and the untrusted-input handling. §14 — added window-layout persistence as an optional future extension with its re-clamping caveat.
